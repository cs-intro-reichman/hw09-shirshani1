...
public class LanguageModel {
    ...
    public void train(String fileName) {
        ...
        while (!in.isEmpty()) {
            ...
            List chance = CharDataMap.get(Window);
            if (chance == null) {
                chance = new List();
                CharDataMap.put(Window, chance);
            }
            chance.update(Char);
            Window = Window + Char;
    ...
    public String generate(String initialText, int textLength) {
        ...
        String generatedT = window;
        for (int i = 0; i < textLength; i++) {
            List probs = CharDataMap.get(window);
            if (probs != null) {
                char newChar = getRandomChar(probs);
                generatedT = generatedT + newChar;
                window = generatedT.substring(generatedT.length() - windowLength);
            } else {
                return generatedT;
            }
        }
